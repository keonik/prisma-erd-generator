...
test('issue 219: unnecessary many-to-many relation lines', async () => {
    ...
    const zeroOrMoreStartUsages = (
        svgContent.match(/marker-start="url\(#[^"]*zeroOrMoreStart[^"]*\)"/g) ||
        []
    ).length
    const onlyOneEndUsages = (
        svgContent.match(/marker-end="url\(#[^"]*onlyOneEnd[^"]*\)"/g) || []
    ).length
    ...
    const relationshipPaths = (
        svgContent.match(/class="[^"]*relationshipLine[^"]*"/g) || []
    ).length
