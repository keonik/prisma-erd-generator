...
- acc6be1: Add a `renderer` option, with a browser-free `graphviz` renderer (#102)
...
- 15e1805: Emit plain SVG text instead of embedded HTML, so diagrams open outside a browser (#245)
...
- 0876f24: Find the mermaid CLI on Windows when it was installed by bun
...
- b834e77: Fix field-level `@map` being applied to the wrong field.
...
- aba3450: Fix composite types and remove stray debug output.
